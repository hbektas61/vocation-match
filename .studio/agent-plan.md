...
| H4 Pilot hardening | H-401–H-411 | **done** — `scripts/check.sh` green: 335 pgTAP assertions across 15 suites, 13 concurrency checks, performance smoke, migration replay, contract check, auth config, dependency gate, tsc, eslint, 226 jest tests, web bundle. Independent security, accessibility and performance passes; the critical finding (a live presence oracle in `swipe` and in the photo read) closed in `20260725002100`/`20260725002200` and recorded as D-016. |
