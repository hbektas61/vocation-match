...
async (page) => {
  ...
  await type('auth-phone', '5551110007');
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (now.includes('profile-name')) {
      ...
      // The passions step wants five, not one.
      const interests = now.filter((id) => id && id.startsWith('interest-'));
      if (interests.length > 0) {
        for (const one of interests.slice(0, 5)) {
          await page.locator(`[data-testid="${one}"]`).first().click();
          await page.waitForTimeout(60);
        }
      } else {
        const pick = now.find((id) => /^(gender-(?!.*self-describe)|orientation-|show-me-)/.test(id)
          && id !== 'show-gender');
