...
async (page) => {
  ...
  await type('auth-phone', '5551110003');
  ...
  // Straight on to an event's detail screen.
  const tap2 = async (id) => { const e = page.locator(`[data-testid="${id}"]`).first(); await e.waitFor({state:'visible',timeout:10000}); await e.click(); };
  const type2 = async (id, v) => { const e = page.locator(`[data-testid="${id}"]`).first(); await e.waitFor({state:'visible',timeout:10000}); await e.click(); await e.pressSequentially(v,{delay:25}); };
  await tap2('tab-Events');
  await page.waitForTimeout(800);
  await type2('events-area-input', 'İstanbul');
  await tap2('events-area-confirm');
  await page.waitForTimeout(1800);
  await tap2('events-upcoming-option-0');
  await page.waitForTimeout(1500);
  await page.screenshot({ path: '/Users/hamibektas/Documents/Codex/2026-07-23/claudeme-sistem-kurmak-istiyorum-cunku-bu/.claude/worktrees/pilot-hardening/.playwright-mcp/app-E-21-fixed.png', scale: 'css', type: 'png' });
